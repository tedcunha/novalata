...
Private Sub Command6_Click()
    ...
    With grdPROGRAMACAO
        ...
        lngQTDSEL = 0
        For I = 1 To (.Rows - 1)
            If .Cell(flexcpChecked, I, conCOL_PROG_Marca) = 1 Then lngQTDSEL = (lngQTDSEL + 1)
        Next I
        If lngQTDSEL > 1 Then
            MsgBox "ATENÇÂO" & vbCrLf & _
        ...
        If .Cell(flexcpChecked, .Row, conCOL_PROG_Marca) = 1 Then
            If Len(Trim(.Cell(flexcpText, .Row, conCOL_PROG_CODOP))) > 0 Then
                ...
                strINDICE = Trim(.Cell(flexcpText, .Row, conCOL_PROG_IDLINHA)) & Trim(Str(Day(dtDATAPROG)) & Trim(Str(Month(dtDATAPROG)))) & Trim(Str(Year(dtDATAPROG))) & Trim(.Cell(flexcpText, .Row, conCOL_PROG_CODGRPLIN))
...
Private Sub ConfGrdLinha()
    ...
    With grdLinha
       ...
       .ColWidth(conCOL_LINHA_DESCLIN) = 1500
       ...
       .ColWidth(conCOL_LINHA_TOTALPROG) = 1000
       .ColWidth(conCOL_LINHA_TOTALDISP) = 1000
...
Private Sub PopGrdDiasProd(lngIDINTERNO As Long)
    ...
    sSql = sSql & " Where" & vbCrLf
    sSql = sSql & "        SGI_FILIAL      = " & FILIAL & vbCrLf
    sSql = sSql & "    And SGI_IDPAI       = " & lngIDINTERNO & vbCrLf
    ...
    If Not BREC2.EOF() Then
        With grdDIASPROG
            Do While Not BREC2.EOF()
                ...
                strINDICE = Trim(Str(lngIDINTERNO)) & Trim(Str(Day(BREC2!SGI_DATA)) & Trim(Str(Month(BREC2!SGI_DATA)))) & Trim(Str(Year(BREC2!SGI_DATA))) & Trim(Str(BREC2!SGI_CODIGO))
...
Private Sub MostraDiasProg()
    With grdLinha
        ...
        If .Row = 0 Then Exit Sub
        Call objBLBFunc.CarregaDadosGrdFilhoSemAction2Do(grdDIASPROG, conCOL_DTP_IDPAI, .Cell(flexcpText, .Row, conCOL_LINHA_IDINTERNO))
...
Private Sub PopGrdProgramado(strDTINICIAL As String, strDTFINAL As String)
    ...
    sSql = sSql & " Where " & vbCrLf
    sSql = sSql & "       SGI_FILIAl = " & FILIAL & vbCrLf
    sSql = sSql & "   And SGI_DATAPROG between " & strDTINICIAL & " And " & strDTFINAL
...
Private Sub CalcQTdJaProg()
    ...
    For I = 1 To (grdDIASPROG.Rows - 1)
        ...
        With grdPROGRAMACAO
            For j = 1 To (.Rows - 1)
                If .Cell(flexcpText, j, conCOL_PROG_Action2Do) <> dacEnumUpdateAction_delete Then
...
Private Function PegaOP(strCODOP As String) As Boolean
    ...
    If Not BREC4.EOF() Then
        ...
        Else
            ...
            With grdPROGRAMACAO
                 If BREC4!SGI_CODLINPROD = CLng(.Cell(flexcpText, .Row, conCOL_PROG_CODLIN)) Then
                    PegaOP = False
                    lngSALDOP = PegaSaldoOP(.EditText)
        ...
        If lngSALDODISP < 0 Then
            ...
            If intRESP = vbYes Then
                ...
                Do While Month(dtDATAPROG) = lngMESVALIDO
                    dtDATAPROG = (dtDATAPROG + 1)
                    strINDICE = Trim(Str(lngIDINTERNO)) & Trim(Str(Day(dtDATAPROG)) & Trim(Str(Month(dtDATAPROG)))) & Trim(Str(Year(dtDATAPROG))) & Trim(Str(lngGRPLINHA))
    ...
    With grdLinha
        For I = 1 To (.Rows - 1)
            ...
            For j = 1 To (grdDIASPROG.Rows - 1)
                If .Cell(flexcpText, I, conCOL_LINHA_IDINTERNO) = grdDIASPROG.Cell(flexcpText, j, conCOL_DTP_IDPAI) Then
...
Private Sub RemanejaOPs(lngCODOP As Long, strINDICE As String, dtDATAPROG As Date, lngIDINTERNO As Long, lngGRPLINHA As Long, lngORDEM As Long, lngROLATU As Long)
    ...
    Do While Month(dtPROGAMACAO) = lngMESVALIDO
        strINDICE = Trim(Str(lngIDINTERNO)) & Trim(Str(Day(dtPROGAMACAO)) & Trim(Str(Month(dtPROGAMACAO)))) & Trim(Str(Year(dtPROGAMACAO))) & Trim(Str(lngGRPLINHA))
...
Private Sub CarregaOP_NaoApontadas()
    ...
    fraExec.Visible = True
    fraExec.Caption = "[ Aguarde Verificando OP's não apontadas ...]"
