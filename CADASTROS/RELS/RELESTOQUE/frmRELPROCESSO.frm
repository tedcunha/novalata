VERSION 5.00
Begin VB.Form frmRELPROCESSO 
   Caption         =   "Relatório de Cadastro de Processos"
   ClientHeight    =   3795
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7260
   ...
   Begin VB.Frame Frame4 
      ...
      Begin VB.CommandButton Command3 
         Height          =   315
         Left            =   2280
         Picture         =   "frmRELPROCESSO.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   15
         Top             =   720
         Width           =   375
      ...
      Begin VB.CommandButton Command2 
         Height          =   315
         Left            =   2280
         Picture         =   "frmRELPROCESSO.frx":0102
         Style           =   1  'Graphical
         TabIndex        =   13
         Top             =   360
         Width           =   375
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdVoltar 
         ...
         Height          =   615
         Left            =   120
         Picture         =   "frmRELPROCESSO.frx":0204
         Style           =   1  'Graphical
         TabIndex        =   2
         Top             =   240
         Width           =   855
      ...
      Begin VB.CommandButton cmdImpressao 
         ...
         Height          =   615
         Left            =   960
         Picture         =   "frmRELPROCESSO.frx":0306
         Style           =   1  'Graphical
         TabIndex        =   1
   ...
   End
End
Attribute VB_Name = "frmRELPROCESSO"
...
Private Sub Command2_Click()
    ...
    arrTABELA(1) = "Select * from SGI_CADPROCESSO Where SGI_FILIAL = " & FILIAL
    ...
    varRETORNO = objPESQPADRAO.cConnect(cCaminho, Linha, FILIAL, strAcesso, V_Usuario, arrCAMPOS, arrTABELA, "Processos")
...
Private Sub Form_Load()
    ...
    Set objRELPROCESSO = CreateObject("RELESTOQUE.clsRELPROCESSO")
    ...
    objRELPROCESSO.FILIAL = FILIAL
    
    objBLBFunc.LimpaCampos frmRELPROCESSO
    
    Call objRELPROCESSO.PreencheComboProcessoI(cboEspGrpInic)
    Call objRELPROCESSO.PreencheComboProcessoF(cboEspGrpFim)
...
Private Sub txtCodGrpFim_GotFocus()
    objBLBFunc.SelecionaCampos txtCodGrpFim.Name, frmRELPROCESSO
End Sub
...
Private Sub txtCodGrpIni_GotFocus()
    objBLBFunc.SelecionaCampos txtCodGrpIni.Name, frmRELPROCESSO
End Sub
...
Private Sub ImpRel()
   ...
   '' Regra para Código
   If Len(Trim(txtCodGrpIni.Text)) > 0 And Len(Trim(txtCodGrpFim.Text)) = 0 Then
      sSql = sSql & "   And SGI_CADPROCESSO.SGI_CODIGO = " & Trim(txtCodGrpIni.Text)
   End If
   If Len(Trim(txtCodGrpIni.Text)) > 0 And Len(Trim(txtCodGrpFim.Text)) > 0 Then
      sSql = sSql & "   And (SGI_CADPROCESSO.SGI_CODIGO >= " & Trim(txtCodGrpIni.Text) & " And SGI_CADPROCESSO.SGI_CODIGO <= " & Trim(txtCodGrpFim.Text) & ")"
   End If
   
   '' Regra para Descrição
   If Len(Trim(cboEspGrpInic.Text)) > 0 And Len(Trim(cboEspGrpFim.Text)) = 0 Then
      sSql = sSql & "   And SGI_CADPROCESSO.SGI_DESCRI LIKE '" & Trim(cboEspGrpInic.Text) & "%'"
   End If
   If Len(Trim(cboEspGrpInic.Text)) > 0 And Len(Trim(cboEspGrpFim.Text)) > 0 Then
      sSql = sSql & "   And (SGI_CADPROCESSO.SGI_DESCRI  >= '" & Trim(cboEspGrpInic.Text) & "' And SGI_CADPROCESSO.SGI_DESCRI <= '" & Trim(cboEspGrpFim.Text) & "')"
   End If
   
   If optOrdSubGrp(0).Value = True Then
      sSql = sSql & " Order by SGI_CADPROCESSO.SGI_CODIGO"
      strCABEC2 = "Relatório de Processos por Ordem de " & optOrdSubGrp(0).Caption
   ElseIf optOrdSubGrp(1).Value = True Then
      sSql = sSql & " Order by SGI_CADPROCESSO.SGI_DESCRI"
      strCABEC2 = "Relatório de Processos por Ordem de " & optOrdSubGrp(1).Caption
   End If
   ...
   If optAgrup(0).Value = True Then '' Normal
     objREL.REL FILIAL, sSql, strCamRelNovo & cCamRelEstoque & "RELPROCESSO.rpt", Linha, 1, strCABEC2, "", True
   ElseIf optAgrup(1).Value = True Then '' Por Agrupamento Seção
     strCABEC3 = "( Por Agrupamento de Espécie de Produtos )"
     objREL.REL FILIAL, sSql, strCamRelNovo & cCamRelEstoque & "RELPROCESSOSEC.rpt", Linha, 1, strCABEC2, strCABEC3, True
   End If
