VERSION 5.00
Begin VB.Form frmRELUNIMEDIDA 
   Caption         =   "Relatório de Unidade de Medidas"
   ClientHeight    =   3195
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7095
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdImpressao 
         ...
         Height          =   615
         Left            =   960
         Picture         =   "frmRELUNIMEDIDA.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   2
      ...
      Begin VB.CommandButton cmdVoltar 
         ...
         Height          =   615
         Left            =   120
         Picture         =   "frmRELUNIMEDIDA.frx":0102
         Style           =   1  'Graphical
         TabIndex        =   1
         Top             =   240
         Width           =   855
   ...
   End
End
Attribute VB_Name = "frmRELUNIMEDIDA"
...
Private Sub Form_Load()
    ...
    Set objREUNIMEDIDA = CreateObject("RELESTOQUE.clsRELUNIMEDIDA")
    ...
    objREUNIMEDIDA.FILIAL = FILIAL
    
    objBLBFunc.LimpaCampos frmRELUNIMEDIDA
...
Private Sub ImpRel()
   ...
   sSql = sSql & " Where "
   sSql = sSql & "       SGI_CADUNIMED.SGI_FILIAL = " & FILIAL
   
   '' Regra para Código
   If Len(Trim(txtCodInic.Text)) > 0 And Len(Trim(txtCodFim.Text)) = 0 Then
      sSql = sSql & "   And SGI_CADUNIMED.SGI_CODIGO = " & Trim(txtCodInic.Text)
   End If
   If Len(Trim(txtCodInic.Text)) > 0 And Len(Trim(txtCodFim.Text)) > 0 Then
      sSql = sSql & "   And (SGI_CADUNIMED.SGI_CODIGO >= " & Trim(txtCodInic.Text) & " And SGI_CADUNIMED.SGI_CODIGO <= " & Trim(txtCodFim.Text) & ")"
   End If
   ...
   If optOrcem(0).Value = True Then
      sSql = sSql & " Order by SGI_CADUNIMED.SGI_CODIGO"
      strCABEC2 = "Relatório de Unidade de Medidas por Ordem de " & optOrcem(0).Caption
   ElseIf optOrcem(1).Value = True Then
      sSql = sSql & " Order by SGI_CADUNIMED.SGI_DESCRICAO"
      strCABEC2 = "Relatório de Unidade de Medidas por Ordem de " & optOrcem(1).Caption
   End If
   ...
   objREL.REL FILIAL, sSql, strCamRelNovo & cCamRelEstoque & "RELUNIMEDIDA.rpt", Linha, 1, strCABEC2, "", True
