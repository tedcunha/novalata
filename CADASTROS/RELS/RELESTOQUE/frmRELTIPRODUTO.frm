VERSION 5.00
Begin VB.Form frmRELTIPRODUTO 
   Caption         =   "Relatório de Tipos de Produtos"
   ClientHeight    =   3240
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7065
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton cmdVoltar 
         ...
         Height          =   615
         Left            =   120
         Picture         =   "frmRELTIPRODUTO.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   15
         Top             =   240
         Width           =   855
      ...
      Begin VB.CommandButton cmdImpressao 
         ...
         Height          =   615
         Left            =   960
         Picture         =   "frmRELTIPRODUTO.frx":0102
         Style           =   1  'Graphical
         TabIndex        =   14
   ...
   End
End
Attribute VB_Name = "frmRELTIPRODUTO"
...
Private Sub Form_Load()
    ...
    Set objRELTIPRODUTO = CreateObject("RELESTOQUE.clsRELTIPRODUTO")
    ...
    objRELTIPRODUTO.FILIAL = FILIAL
    
    objBLBFunc.LimpaCampos frmRELTIPRODUTO
...
Private Sub ImpRel()
   ...
   sSql = sSql & "      ,SGI_CADTIPPROD.SGI_TEMLST "
   sSql = sSql & "      ,SGI_CADTIPPROD.SGI_COMPLST "
   ...
   '' Regra para Código
   If Len(Trim(txtCodInic.Text)) > 0 And Len(Trim(txtCodFim.Text)) = 0 Then
      sSql = sSql & "   And SGI_CADTIPPROD.SGI_CODIGO = " & Trim(txtCodInic.Text)
   End If
   If Len(Trim(txtCodInic.Text)) > 0 And Len(Trim(txtCodFim.Text)) > 0 Then
      sSql = sSql & "   And (SGI_CADTIPPROD.SGI_CODIGO >= " & Trim(txtCodInic.Text) & " And SGI_CADTIPPROD.SGI_CODIGO <= " & Trim(txtCodFim.Text) & ")"
   End If
   
   '' Regra para Descrição
   If Len(Trim(txtDescIni.Text)) > 0 And Len(Trim(txtDescFim.Text)) = 0 Then
      sSql = sSql & "   And SGI_CADTIPPROD.SGI_DESCRICAO LIKE '" & Trim(txtDescIni.Text) & "%'"
   End If
   If Len(Trim(txtDescIni.Text)) > 0 And Len(Trim(txtDescFim.Text)) > 0 Then
      sSql = sSql & "   And (SGI_CADTIPPROD.SGI_DESCRICAO  >= '" & Trim(txtDescIni.Text) & "' And SGI_CADTIPPROD.SGI_DESCRICAO <= '" & Trim(txtDescFim.Text) & "')"
   End If
   
   If optOrcem(0).Value = True Then
      sSql = sSql & " Order by SGI_CADTIPPROD.SGI_CODIGO"
      strCABEC2 = "Relatório de Tipo de Produtos por Ordem de " & optOrcem(0).Caption
   ElseIf optOrcem(1).Value = True Then
      sSql = sSql & " Order by SGI_CADTIPPROD.SGI_DESCRICAO"
      strCABEC2 = "Relatório de Tipo de Produtos por Ordem de " & optOrcem(1).Caption
   End If
   ...
   objREL.REL FILIAL, sSql, strCamRelNovo & cCamRelEstoque & "RELTIPRODUTO.rpt", Linha, 1, strCABEC2, "", True
