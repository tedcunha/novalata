...
Begin VB.Form frmCADCOTAS 
   ...
   Begin VB.Frame Frame26 
      ...
      Begin VSFlex8LCtl.VSFlexGrid grdSEMANAS 
         ...
         AccessibleRole  =   24
      End
      Begin MSComCtl2.MonthView mtvCalendario 
         Height          =   2310
         Left            =   3960
         TabIndex        =   27
         Top             =   3720
...
Private Sub PegaDataLivre(strMES As String, strANO As String, strIDPRODUTO As String)
    ...
    sSql = sSql & "      PMDIA.SGI_DATA AS SGI_DTSEMANA" & vbCrLf
    ''sSql = sSql & "     ,PMDIA.SGI_DIASEMANA" & vbCrLf
    sSql = sSql & "     ,Sum(PMDIA.SGI_TOTALPECAS) As SGI_QTDE" & vbCrLf
    sSql = sSql & "     ,GRPI.SGI_CODLIN" & vbCrLf
    sSql = sSql & "     ,PROD.SGI_NECKIN" & vbCrLf
    sSql = sSql & "     ,GLIN.SGI_ATIVO" & vbCrLf
    ...
    ''sSql = sSql & "        PMDIA.SGI_SEMANA" & vbCrLf
    sSql = sSql & "        PMDIA.SGI_DATA" & vbCrLf
    ''sSql = sSql & "       ,PMDIA.SGI_DIASEMANA" & vbCrLf
    sSql = sSql & "       ,GRPI.SGI_CODLIN" & vbCrLf
    sSql = sSql & "       ,PROD.SGI_NECKIN" & vbCrLf
    sSql = sSql & "       ,GLIN.SGI_ATIVO" & vbCrLf
    ...
    If Not BREC.EOF() Then
        ...
        Else
            With grdSEMANAS
                Do While Not BREC.EOF()
                    ...
                    mtvCalendario.Value = CDate(Format(BREC!SGI_DTSEMANA, "DD/MM/YYYY"))
                    lngSemana = mtvCalendario.Week
                    
                    lngLINHA = .FindRow(lngSemana, , conCOL_SonMes_Semana)
                    If lngLINHA < 0 Then
                    ...
                    If BREC!SGI_DTSEMANA >= dtDATA Then
                        ...
                        lngLINHA = .FindRow(lngSemana, , conCOL_SonMes_Semana)
                        If lngLINHA > 0 Then
                            intDiaSemana = Weekday(BREC!SGI_DTSEMANA)
                           .Cell(flexcpText, lngLINHA, intDiaSemana) = Day(BREC!SGI_DTSEMANA)
                           Call PintaCelula(BREC!SGI_CODLIN, Format(BREC!SGI_DTSEMANA, "DD/MM/YYYY"), lngLINHA, intDiaSemana, BREC!SGI_QTDE, BREC!SGI_NECKIN, intHOMOLOGADO)
                        End If
                    End If
        ...
        End If
    Else
        MsgBox "ATENÇÃO - Não estÁ cadastrado calendário para esta linha !!!" & vbCrLf & _
               "FAVOR INFORMAR O PCP", vbOKOnly + vbExclamation, "Aviso"
    End If
    BREC.Close
...
On Error GoTo Err_grdSEMANAS_DblClick
    ...
    If (grdSEMANAS.Rows - 1) = 0 And grdSEMANAS.RowSel = 0 Then Exit Sub
    ...
    With grdSEMANAS
        ...
        Case conCOL_SonMes_Domingo, _
             ...
             Else
                ...
                strDATA = Format(.Cell(flexcpText, .Row, .Col), "##00") & "/" & Format(cboMes.ItemData(cboMes.ListIndex), "##00") & "/" & cboAno.ItemData(cboAno.ListIndex)
                ...
                '' ===============
                '' dia 03/08/2017
                '' Mudança dos status P.Cota e P.Data , agora todos serão liberado, não ira mais controlar P.Cota / P.Data
                '' foi mudado
                '' intStatusOP = 7  -> P.Data
                '' intStatusOP = 6  -> P.Cota
                '' Ambos será intStatusOP = 0 -> Independente da Data e Cota
                '' ===============
                ...
                If CDate(strDATA) < dtDTLIDTIME Then
                    ...
                    ''intStatusOP = 7 -> 03/08/2017
                    intStatusOP = 0
                    strRETORNO = strDATA
                    Unload Me
                ElseIf CotaEstourada(strPRODCODLIN, strDATA) = True Then
                    '' P.Cota
                    '' Status = 6
                    If intAction2Do = dacEnumUpdateAction_Ignore Then intAction2Do = dacEnumUpdateAction_update
                    ''intStatusOP = 6 -> 03/08/2017
                    intStatusOP = 0
                    strRETORNO = strDATA
                    Unload Me
                Else
...
''    sSql = sSql & "     ,(" & objCADPEDVENDACOTA.PegaPedQueryBloqPcPd("LIMP.SGI_CODLIN", strDTENTREGA, strNOMFILIAL, lngCODPED, lngNECKIN, intHOMOLOGADO) & ") As QTDEPEDBLOQPCPD" & vbCrLf
    ...
    If Not BREC7.EOF() Then
        Do While Not BREC7.EOF()
            ...
            lngQTDEOP = objCADPEDVENDACOTA.PegaQueryOPDia(BREC7!SGI_CODLIN, strDTENTREGA, strNOMFILIAL, lngCODPED, lngNECKIN, intHOMOLOGADO)
            lngQTDEPEDBLOQ2 = 0
            lngQTDEPEDBLOQALT2 = 0
            lngQTDEPEDLBOQFOT2 = 0
            lngQTDEPEDBLOQPCPD2 = objCADPEDVENDACOTA.PegaPedQueryBloqPcPd(BREC7!SGI_CODLIN, strDTENTREGA, strNOMFILIAL, lngCODPED, lngNECKIN, intHOMOLOGADO)
...
Private Sub MostraDadosCotas()
    ...
    If Len(Trim(grdSEMANAS.Cell(flexcpData, grdSEMANAS.Row, grdSEMANAS.Col))) > 0 And Len(Trim(grdSEMANAS.Cell(flexcpText, grdSEMANAS.Row, grdSEMANAS.Col))) > 0 Then
        ...
        If Len(Trim(strIDINTERNO)) > 0 Then
        ...
        If IsArray(arrDADOS) Then
            ...
            lblDADOS(12).Caption = lngTOTALATRAZADO '' Em Atrazo
            lblDADOS(2).Caption = arrDADOS(1)       '' OP's Alocadas
            ''lblDADOS(3).Caption = arrDADOS(2)       '' Pedidos Bloqueados
            ''lblDADOS(4).Caption = arrDADOS(3)       '' Bloqueado P/Alteração
            ''lblDADOS(5).Caption = arrDADOS(4)       '' Aguardando Liberação de Fotolito
            lblDADOS(6).Caption = arrDADOS(5)       '' P.Cota / P.Data
